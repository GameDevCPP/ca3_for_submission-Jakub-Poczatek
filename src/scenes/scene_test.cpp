...
void LevelTestScene::Load() {
    ...
    for(auto tile: ls::findTiles(Color(ls::FENEMY))){
        auto enemy = makeEntity();
        enemy->setPosition(ls::getTilePosition(tile) +
                   Vector2f((float) JsonData::generalData["tileSize"] - (float) JsonData::enemies["fEnemy"]["radius"],
                            (float) JsonData::generalData["tileSize"] - (float) JsonData::enemies["fEnemy"]["radius"]));
        enemy->addComponent<HurtComponent>();
        auto s = enemy->addComponent<ShapeComponent>();
